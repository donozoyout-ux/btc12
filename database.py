...
class Database:
    ...
    def get_daily_pnl(self, days=14):
        from datetime import datetime, timedelta
        conn = self._conn()
        try:
            rows = conn.execute("""
                SELECT substr(created_at, 1, 10) as gun, ROUND(SUM(pnl), 4), COUNT(*)
                FROM trades
                WHERE action='SELL' AND created_at IS NOT NULL AND substr(created_at, 1, 10) != ''
                GROUP BY gun
            """).fetchall()
            by_date = {r[0]: {"pnl": r[1] or 0, "count": r[2]} for r in rows if r[0]}
            today = datetime.now().date()
            result = []
            for i in range(days - 1, -1, -1):
                day = (today - timedelta(days=i)).isoformat()
                entry = by_date.get(day, {"pnl": 0, "count": 0})
                result.append({"date": day, "pnl": entry["pnl"], "count": entry["count"]})
            return result
        finally:
            conn.close()
