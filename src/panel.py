...
from core import get_account, get_positions, get_position, buy, sell, sell_all

app = Flask(__name__)
app.config['SECRET_KEY'] = 'crypto-scanner'
...
<h1 class="text-lg font-bold tracking-tight text-white">CRYPTO BOT</h1>
...
<button onclick="switchTab('manual')" id="tabManual" class="text-xs font-bold text-text3 border-b-2 border-transparent pb-1 cursor-pointer">MANUEL ISLEM</button>
</div>
...
<thead class="bg-surface3/50 border-b border-border/20">
<tr>
<th class="px-4 py-3 w-10"><input type="checkbox" id="selectAllCb" class="sell-cb" onchange="toggleAll(this)"></th>
<th class="px-4 py-3 text-[10px] font-bold tracking-widest text-text3">COIN</th>
...
<div id="manualPanel" class="flex flex-col flex-1 hidden">
<h2 class="text-sm font-bold text-white mb-4">Manuel Islem</h2>
<div class="grid grid-cols-2 gap-4">
<div class="glass rounded-lg p-6">
<h3 class="text-lg font-bold text-green mb-4">ALIS</h3>
<div class="mb-4">
<label class="text-xs text-text3 font-bold">COIN</label>
<select id="buyCoin" class="w-full mt-1 bg-surface4 border border-border/30 rounded px-3 py-2 text-white font-mono text-sm">
<option value="BTC/USD">BTC/USD</option>
<option value="ETH/USD">ETH/USD</option>
</select>
</div>
<div class="mb-4">
<label class="text-xs text-text3 font-bold">MIKTAR ($)</label>
<input id="buyAmount" type="number" value="300" class="w-full mt-1 bg-surface4 border border-border/30 rounded px-3 py-2 text-white font-mono text-sm">
</div>
<button onclick="manualBuy()" class="btn w-full py-3 bg-green hover:bg-green/80 text-black font-bold rounded">ALIS YAP</button>
</div>
<div class="glass rounded-lg p-6">
<h3 class="text-lg font-bold text-red mb-4">SATIS</h3>
<div id="sellPositions" class="space-y-2 mb-4">
<p class="text-text3 text-sm">Pozisyon yok</p>
</div>
<button onclick="sellAll()" class="btn w-full py-3 bg-red hover:bg-red/80 text-white font-bold rounded">HEPSINI SAT</button>
</div>
</div>
</div>
</div>
...
function switchTab(t){
    currentTab=t;
    ['scan','positions','manual'].forEach(x=>{
        document.getElementById('tab'+x.charAt(0).toUpperCase()+x.slice(1)).className =
            x===t?'text-xs font-bold text-white border-b-2 border-accent pb-1 cursor-pointer'
                  :'text-xs font-bold text-text3 border-b-2 border-transparent pb-1 cursor-pointer';
        document.getElementById(x+'Panel').className = x===t?'flex flex-col flex-1':'flex flex-col flex-1 hidden';
    });
    if(t==='manual') loadSellPositions();
}
...
function manualBuy(){
    var sym=document.getElementById('buyCoin').value;
    var amt=document.getElementById('buyAmount').value;
    if(!confirm(sym+' icin $'+amt+' degerinde alis yapilsin mi?'))return;
    fetch('/api/buy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({symbol:sym,amount:parseFloat(amt)})})
    .then(r=>r.json()).then(d=>{toast(d.message,d.success?'success':'error');setTimeout(refresh,3000)});
}
...
function loadSellPositions(){
    fetch('/api/status').then(r=>r.json()).then(d=>{
        var positions=d.positions||[];
        var el=document.getElementById('sellPositions');
        if(positions.length===0){el.innerHTML='<p class="text-text3 text-sm">Pozisyon yok</p>';return;}
        var html='';
        positions.forEach(p=>{
            var pl=p.unrealized_pl||0;
            var plColor=pl>=0?'text-green':'text-red';
            html+='<div class="flex items-center justify-between bg-surface4 rounded px-3 py-2">';
            html+='<div><span class="font-bold">'+p.symbol+'</span> <span class="text-xs text-text3">'+p.qty.toFixed(6)+'</span></div>';
            html+='<div class="flex items-center gap-3"><span class="'+plColor+' font-mono text-sm">$'+pl.toFixed(2)+'</span>';
            html+='<button onclick="sellCoin(\\''+p.symbol+'\\')" class="btn px-3 py-1 bg-red/20 hover:bg-red text-red hover:text-white text-[10px] font-bold rounded">SAT</button></div>';
            html+='</div>';
        });
        el.innerHTML=html;
    });
...
function updateStatus(){
    fetch('/api/status').then(r=>r.json()).then(d=>{
        ...
        document.getElementById('scanStatus').textContent=d.running?(d.paused?'DURAKLATILDI':'Tarama aktif - '+d.symbols.length+' coin'):'Durdu';
...
def api_start():
    ...
    bot.start()
    return jsonify({"success": True, "message": f"Bot baslatildi - {', '.join(SYMBOLS)}"})
...
def api_buy():
    try:
        ...
        amount = data.get("amount", POSITION_SIZE)
        result = buy(sym, round(amount / get_price(sym), 6))
        return jsonify({
            "success": True,
